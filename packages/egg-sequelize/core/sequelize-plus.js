...
module.exports = (app) => {
  ...
  const config = app.config.sequelizePlus
  ...
  function loadDatabase(config = {}) {
    if (typeof config.ignore === 'string' || Array.isArray(config.ignore)) {
      app.deprecate(`[egg-sequelize-plus] if you want to exclude ${
        config.ignore
    ...
    const sequelize = config.connectUri != null
      ? new app.Sequelize(config.connectUri, config)
    ...
    if (model[delegate[len - 1]]) {
      throw new Error(`[egg-sequelize-plus] app[${config.delegate}] is already defined`)
    }
